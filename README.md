...
The **Unity Game Client** integrates with the holdem-processor API for live table state and runs as a Unity project (WebGL or Editor) alongside the existing serverless stack.
...
| Service | URL |
|---------|-----|
| Rewards API health | `http://localhost:<REWARDS_API_PORT>/api/v1/health` (see `.env`; use `5500` on macOS if AirPlay uses `5000`) |
...
Compose uses the project name **`hijack-poker`** (set at the top of `docker-compose.yml`), so containers are named like `hijack-poker-mysql-1`. If you previously used an auto-generated project name and hit a broken Compose “recreate” state, run `docker compose down` from this repo once; the fixed name prevents that class of stale-container errors.
...
│           + Hand Viewer (:8080)                                  │
│                                                                  │
│  rewards: core + Rewards API (host :REWARDS_API_PORT) + React (:4000) │
│                                                                  │
...
- **Rewards panel** (when the rewards API is running): live tier, monthly points, multiplier, lifetime/next tier, and recent point events—same backend and player id as the React rewards dashboard. Optional query params: `?player=player-001&rewardsApi=http://localhost:5000&dashboard=http://localhost:4000` (defaults match local compose; use your `REWARDS_API_PORT` if it is not 5000).
...
| Rewards API | `REWARDS_API_PORT` in `.env` (default 5000; 5500 on macOS if 5000 busy) | rewards |
...
On **macOS**, **AirPlay Receiver** often listens on **5000**, which blocks Docker from publishing the Rewards API on that port. Set `REWARDS_API_PORT=5500` (or another free port) in `.env`, then use `http://localhost:5500/api/v1/health` for health checks.
